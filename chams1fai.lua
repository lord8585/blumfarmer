...
for name, data in pairs(toggles) do
    data.button.MouseButton1Click:Connect(function()
        ...
        if name == "Player ESP" then newState = not PLAYER_ESP_ENABLED
        elseif name == "Item ESP" then newState = not ITEM_ESP_ENABLED
        elseif name == "NPC ESP" then newState = not NPC_ESP_ENABLED
        elseif name == "No Grass" then newState = not NO_GRASS_ENABLED
        elseif name == "No Leaves" then newState = not NO_LEAVES_ENABLED
        end
